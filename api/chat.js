...
新增身份：你也是用户的智能管家。你要识别用户什么时候是在控制设备，什么时候是在聊天。用户说“开灯”“把空调调到26度”“关闭客厅的窗帘”这类设备控制话时，必须调用 control_device 工具，绝对不能假装已经执行或编造执行结果；用户说“我心情不好”“讲个笑话”“陪我聊聊”时正常聊天。用户询问天气、气温、是否下雨等需要实时信息时，必须调用 get_weather 工具，绝对不能凭常识或想象编造天气、温度、降雨、风速等实时信息。工具执行后，用自然、亲近但克制的中文回复，不要每次都机械地说“好的我来执行”。
...
async function getWeather(location) {
  ...
  const key = provider === "qweather"
    ? qweatherKey || weatherApiKey
    : provider === "openweather"
      ? openWeatherKey || weatherApiKey
      : weatherApiKey || qweatherKey || openWeatherKey;
  if (!key) return "天气 API Key 还没有配置。请在 Vercel 环境变量里设置 WEATHER_API_KEY。";

  if (provider === "qweather" || (!provider && qweatherKey)) {
    ...
    if (!city) return `我暂时没查到 ${place} 的天气。`;
    ...
    if (!now) return `我暂时没拿到 ${city.name} 的实时天气。`;
    return `${city.name}现在${now.text}，气温 ${now.temp}°C，体感 ${now.feelsLike}°C，风速 ${now.windSpeed} km/h。`;
  }

  if (provider === "openweather" || (!provider && openWeatherKey)) {
    ...
    if (!main) return `我暂时没拿到 ${city.local_names?.zh || city.name} 的实时天气。`;
    return `${city.local_names?.zh || city.name}现在${weatherData.weather?.[0]?.description || "天气变化中"}，气温 ${main.temp}°C，体感 ${main.feels_like}°C，风速 ${weatherData.wind?.speed ?? "未知"} m/s。`;
  }

  const weatherUrl = `https://api.weatherapi.com/v1/current.json?key=${key}&q=${encodeURIComponent(place)}&lang=zh`;
  const weatherData = await fetch(weatherUrl).then((response) => response.json()).catch(() => ({}));
  const current = weatherData?.current;
  if (!current) return `我暂时没拿到 ${place} 的实时天气。`;

  return `${weatherData.location?.name || place}现在${current.condition?.text || "天气变化中"}，气温 ${current.temp_c}°C，体感 ${current.feelslike_c}°C，风速 ${current.wind_kph} km/h。`;
}
